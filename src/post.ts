...
export class TwitterPostClient {
  ...
  constructor(client: ClientBase, runtime: IAgentRuntime) {
    ...
    elizaLogger.debug(
      `🔍 Raw approval provider from settings: "${rawApprovalProvider}"`
    );

    this.approvalProvider = rawApprovalProvider || "RAIINMAKER";
    elizaLogger.debug(
      `🔍 Final approval provider set to: "${this.approvalProvider}"`
    );
    ...
    const approvalEnabledSetting = this.runtime.getSetting(
      "TWITTER_APPROVAL_ENABLED"
    );
    elizaLogger.debug(
      `🔍 TWITTER_APPROVAL_ENABLED setting: "${approvalEnabledSetting}"`
    );

    const approvalRequired: boolean =
      approvalEnabledSetting?.toLowerCase() === "true";
    ...
    if (approvalRequired) {
      ...
      const approvalCheckIntervalSetting = this.runtime.getSetting(
      ...
      elizaLogger.log(
        `Twitter approval enabled using ${
          this.approvalProvider
        } verification with ${
          this.approvalCheckInterval / 1000
        }s check interval`
      );

      elizaLogger.debug(
        `🔍 Checking provider - current provider: "${this.approvalProvider}"`
      );
      ...
      if (this.approvalProvider === "RAIINMAKER") {
        ...
        elizaLogger.debug(
          `🔍 Raiinmaker plugin available: ${raiinmakerEnabled}`
        );

        if (!raiinmakerEnabled) {
          elizaLogger.warn(
            "Twitter approval is set to use Raiinmaker but the plugin is not available"
          );
          elizaLogger.debug(
            `🔍 Setting this.approvalRequired = false due to missing Raiinmaker plugin`
          );
          this.approvalRequired = false;
        } else {
          // Skip Discord setup completely for Raiinmaker provider
          elizaLogger.debug(
            `🔍 Skipping Discord setup for Raiinmaker provider`
          );
        ...
        if (!discordToken || !channelId) {
          elizaLogger.warn(
            "Twitter approval is set to use Discord but credentials are missing"
          );
          elizaLogger.debug(
            `🔍 Setting this.approvalRequired = false due to missing Discord credentials`
          );
          this.approvalRequired = false;
        } else {
          elizaLogger.debug(
            `🔍 Setting Discord approval channel ID and initializing Discord client`
          );
        ...
        elizaLogger.debug(
          `🔍 Unknown approval provider: "${this.approvalProvider}"`
        );
      }
    } else {
    ...
    elizaLogger.debug(
      `🔍 TwitterPostClient constructor complete. Final approval provider: "${this.approvalProvider}", approval required: ${this.approvalRequired}`
    );
  }
  ...
  private setupDiscordClient() {
    try {
      ...
      const token = this.runtime.getSetting(
        "TWITTER_APPROVAL_DISCORD_BOT_TOKEN"
      );
      this.discordApprovalChannelId = this.runtime.getSetting(
        "TWITTER_APPROVAL_DISCORD_CHANNEL_ID"
      );

      if (!token || !this.discordApprovalChannelId) {
        elizaLogger.error(
          "Missing required Discord environment variables for Twitter approval"
        );
        this.discordClientForApproval = null;
        return;
      }
  ...
  private async sendForRaiinmakerVerification(
    ...
    try {
      ...
      elizaLogger.log(
        `Sending tweet for Raiinmaker verification: "${tweetTextForPosting.substring(
          0,
          50
        )}${tweetTextForPosting.length > 50 ? "..." : ""}"`
      );
      ...
      try {
        ...
        elizaLogger.error(
          "Failed to create original room for tweet verification:",
          roomError
        );
        ...
        try {
          ...
          elizaLogger.error(
            "Failed to create fallback room for tweet verification:",
            fallbackError
          );
      ...
      try {
        ...
        if (verificationResult?.taskId) {
          ...
          const taskIdMatch = verificationResult.text.match(
            /Task ID: ([a-f0-9-]{36})/i
          );
        ...
        if (!taskId) {
          elizaLogger.error(
            "Failed to create verification task: Invalid response from Raiinmaker plugin"
          );
          ...
          if (this.client.twitterConfig.POST_IMMEDIATELY) {
            elizaLogger.warn(
              "Verification failed, but POST_IMMEDIATELY is enabled - posting tweet directly"
            );
        ...
        elizaLogger.log(
          `Successfully created verification task with ID: ${taskId}`
        );
        ...
        elizaLogger.error(
          "Error executing VERIFY_GENERATION_CONTENT action:",
          actionError
        );
      ...
      elizaLogger.error(
        "Error sending tweet for Raiinmaker verification:",
        error
      );
      ...
      if (this.client.twitterConfig.POST_IMMEDIATELY) {
        elizaLogger.warn(
          "Verification process failed, but POST_IMMEDIATELY is enabled - posting tweet directly"
        );
        ...
        try {
          ...
          elizaLogger.error(
            "Failed to post tweet in fallback mode:",
            postError
          );
          return null;
        }
   ...
   */
  private async checkRaiinmakerVerificationStatus(
    taskId: string
    ...
    try {
      ...
      try {
        ...
        const status =
          typeof verificationStatus.status === "string"
            ? verificationStatus.status.toLowerCase()
            : null;

        const answer =
          typeof verificationStatus.answer === "string"
      ...
      elizaLogger.error(
        `Error checking verification status for task ${taskId}:`,
        error
      );
      return "PENDING";
    }
  ...
  private async sendForDiscordApproval(
    ...
    try {
      elizaLogger.log(
        `Sending tweet for Discord approval: "${tweetTextForPosting.substring(
          0,
          50
        )}${tweetTextForPosting.length > 50 ? "..." : ""}"`
      );
      ...
      if (!this.discordClientForApproval || !this.discordApprovalChannelId) {
        elizaLogger.error(
          "Discord client or channel ID not configured for approval"
        );

        // If Discord approval fails but POST_IMMEDIATELY is enabled, post directly
        if (this.client.twitterConfig.POST_IMMEDIATELY) {
          elizaLogger.warn(
            "Discord approval failed, but POST_IMMEDIATELY is enabled - posting tweet directly"
          );
      ...
      try {
        ...
        if (!channel || !(channel instanceof TextChannel)) {
          throw new Error(
            `Invalid Discord channel: ${this.discordApprovalChannelId}`
          );
        }
        ...
        elizaLogger.success(
          `Successfully sent tweet for Discord approval with message ID: ${message.id}`
        );
        return message.id;
      } catch (error) {
      ...
      if (this.client.twitterConfig.POST_IMMEDIATELY) {
        elizaLogger.warn(
          "Discord approval process failed, but POST_IMMEDIATELY is enabled - posting tweet directly"
        );
  ...
  async start() {
    try {
      ...
      if (this.approvalProvider?.toUpperCase() === "RAIINMAKER") {
        elizaLogger.debug(
          `🔍 Explicitly ensuring Discord client is null for RAIINMAKER provider during start()`
        );
        this.discordClientForApproval = null;
      }
  ...
  private async sendForVerification(
    ...
    if (provider === "RAIINMAKER") {
      this.discordClientForApproval = null;
      return this.sendForRaiinmakerVerification(
        tweetTextForPosting,
        roomId,
        rawTweetContent
      );
      ...
      return this.sendForDiscordApproval(
        tweetTextForPosting,
        roomId,
        rawTweetContent
      );
      ...
      elizaLogger.warn(
        `Unknown provider "${this.approvalProvider}", defaulting to Raiinmaker`
      );
      this.discordClientForApproval = null;
      return this.sendForRaiinmakerVerification(
        tweetTextForPosting,
    ...
    try {
      ...
      try {
        if (this.approvalRequired) {
          ...
          elizaLogger.log(
            `Sending Tweet for ${this.approvalProvider} verification:\n ${tweetTextForPosting}`
          );
          ...
          if (taskId === "direct-posted") {
            elizaLogger.log(
              "Tweet was posted directly due to verification fallback"
            );
          } else if (taskId) {
            elizaLogger.log(
              `Tweet sent for verification with task ID: ${taskId}`
            );
          } else {
  ...
  private async checkVerificationStatus(
    ...
    if (this.approvalProvider === "DISCORD") {
      ...
      elizaLogger.warn(
        `Unknown provider "${this.approvalProvider}", defaulting to PENDING status`
      );
      return "PENDING";
    }
  }
  ...
  private async checkApprovalStatus(
    ...
    try {
      ...
      if (rejectReaction) {
        ...
        if (reactionCount > 1) {
          // More than just the bot's reaction
  ...
  private async cleanupPendingTweet(taskId: string) {
    try {
      ...
      if (updatedPendingTweets.length === 0) {
        ...
        elizaLogger.debug(
          `Updated pending tweets cache, ${updatedPendingTweets.length} tweets remaining`
        );
      }
      ...
      try {
        ...
        elizaLogger.error(
          "Error creating verification tracking memory:",
          roomError
        );
  ...
  private async handlePendingTweet() {
    elizaLogger.log(
      `Checking pending tweets using ${this.approvalProvider} verification...`
    );
    elizaLogger.debug(
      `🔍 handlePendingTweet called with approval provider: "${this.approvalProvider}"`
    );

    // Additional safeguard to prevent Discord initialization for RAIINMAKER
    if (this.approvalProvider.toUpperCase() === "RAIINMAKER") {
      elizaLogger.debug(
        `🔍 Explicitly ensuring Discord client is null for RAIINMAKER provider`
      );
      this.discordClientForApproval = null;
    }
    ...
    for (const pendingTweet of pendingTweets) {
      elizaLogger.log(
        `Processing pending tweet with taskId: ${pendingTweet.taskId}`
      );
      elizaLogger.debug(
        `🔍 Checking tweet with taskId: ${pendingTweet.taskId}`
      );
      ...
      if (isExpired) {
        elizaLogger.warn(
          `Pending tweet with task ID ${pendingTweet.taskId} expired after 24 hours`
        );
      ...
      elizaLogger.log(
        `Checking verification status for task: ${pendingTweet.taskId}`
      );
      elizaLogger.debug(
        `🔍 About to call checkVerificationStatus for task: ${pendingTweet.taskId}`
      );
      const approvalStatus = await this.checkVerificationStatus(
        pendingTweet.taskId
      );
      elizaLogger.log(
        `Approval status for task ${pendingTweet.taskId}: ${approvalStatus}`
      );
      elizaLogger.debug(`🔍 Received approval status: ${approvalStatus}`);

      if (approvalStatus === "APPROVED") {
        elizaLogger.log(
          `Tweet with task ID ${pendingTweet.taskId} approved, posting now...`
        );
        ...
        elizaLogger.warn(
          `Tweet with task ID ${pendingTweet.taskId} rejected by ${this.approvalProvider} verification`
        );
        elizaLogger.debug(`🔍 Tweet rejected, cleaning up`);
        await this.cleanupPendingTweet(pendingTweet.taskId);
      } else {
        elizaLogger.log(
          `Tweet with task ID ${pendingTweet.taskId} still pending verification`
        );
  ...
  private async startVerificationPolling() {
    try {
      ...
      elizaLogger.log(
        `Started ${this.approvalProvider} verification check loop`
      );
    } catch (error) {
